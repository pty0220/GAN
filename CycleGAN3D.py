from keras.layers import Layer, Input, Dropout, Conv3D, Activation, add, BatchNormalization, UpSampling3D, \
    Conv3DTranspose, Flatten, MaxPooling3D, concatenate
...
class CycleGAN():
    def __init__(self):
        ...
        self.root_dir = 'D:/KHK/model/Medical-Image-Synthesis-multiview-Resnet_patch3D/'
        self.retrain_path = False#'20201025-143028-T1-CT_cropped_bias_P5_LR_0.0002_RL_9_DF_64_GF_32_RF_70' #False
        self.retrain_epoch = False #137  # False
        ...
        self.patch_size = data["volume_size_A"][1] // 4  # 64
        self.stride = data["volume_size_A"][1] // 16  # 16
        self.patch_size_test = data["volume_size_A"][1] // 4  # 64
        self.stride_test = data["volume_size_A"][1] // 16  # 16
    ...
    def build_generator_Unet(self, vol_shape_in, vol_shape_out, name=None):
        inputs = Input(shape=vol_shape_in)
        ...
        conv5_1 = BatchNormalization(momentum=0.8)(conv5_1)
        ...
        conv6_1 = BatchNormalization(momentum=0.8)(conv6_1)
        ...
        conv10 = Conv3D(vol_shape_out[-1], (1, 1, 1))(conv9)

        x = Activation('tanh')(conv10)
    ...
    def retrain(self):
        ...
        # load generator B to A
        path_to_weights = os.path.join(self.root_dir, 'runs', self.retrain_path, 'models',
                                       'G_B2A_model_epoch_{}.hdf5'.format(self.retrain_epoch))
        self.G_B2A.load_weights(path_to_weights)

        # load discriminator A
        path_to_weights = os.path.join(self.root_dir, 'runs', self.retrain_path, 'models',
                                       'D_A_model_epoch_{}.hdf5'.format(self.retrain_epoch))
        self.D_A.load_weights(path_to_weights)
    ...
    def train(self, epochs, batch_size=1):
        ...
        def run_training_batch():
            ...
            if self.use_linear_decay and epoch >= self.decay_epoch:
                self.update_lr(self.D_A, decay_D, loop_index, epoch)
    ...
    def slices2nii(self, slices, refer_img, name):
        ...
        for i in range(len(slices)):
            #
            # mae between synthesize and real image
            #
            slice = slices[len(slices) - 1 - i]
            # volume.append(slice[:,:,0].transpose())
            volume.insert(0, slice[:, :, 0])
        #
        # axial slices to dicom
        #
        volume = np.array(volume)
        volume = (volume + 1) / 2
        volume = volume * (3000 - (-1000))
        volume = volume + (-1000)

        start = (256 - 192) // 2
        volume = volume[:, :, start:start + 192]
    ...
    def save_tmp_images(self, real_volume_A, real_volume_B, synthetic_volume_A, synthetic_volume_B):
        ...
        try:
            ...
            self.join_and_save((real_images_B, synthetic_images_A, real_images_A), save_path)
    ...
    def save_epoch_volumes(self, epoch, num_saved_volumes=1):
        ...
        recon_real_volume_B = self.reconstruct_from_patches(real_volume_B[:, :, :, :, 0], (256, 256, 256), self.stride)
        recon_synthetic_volume_B = self.reconstruct_from_patches(synthetic_volume_B[:, :, :, :, 0], (256, 256, 256),
                                                                 self.stride)
